...
export async function getServerSideProps({ params }) {
  const exhibit = await fetcher(`${process.env.API_URL}/exhibit/${params.id}/`);
  return {
    props: {
      fallback: {
        [`${process.env.API_URL}/exhibit/${exhibit.id}/`]: exhibit,
      },
    },
  };
}
...
function Content() {
  ...
  const { data: exhibit } = useSWR(
    `${process.env.API_URL}/exhibit/${query.id}/`
  );

  const { data: last_exhibits } = useSWR(
    `${process.env.API_URL}/exhibits/last/`,
    fetcher
  );

  return (
    <Layout title={exhibit.name}>
                ...
                <div className="row" data-container="container">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <div className="moto-widget-text-content moto-widget-text-editable">
                          <h1 className="moto-text_system_8">{exhibit.name}</h1>
                        </div>
                      </div>
                    </div>
                    <div
                      ...
                      >
                        <div className="container-fluid">
                          <div className="row moto-widget-blog-recent_posts-list">
                            {last_exhibits ? (
                              <>
                                {last_exhibits.map((last_exhibit) => (
                                  <div
                                    key={last_exhibit.id}
...
export default function Exhibit({ fallback }) {
  return (
